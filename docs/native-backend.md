...
- dependency injection: a class's `deps { d: I }` are stored in its instance and constructed recursively on resolve; a method reaches a dependency by its bare name; state is zero-initialized (so `""`, `[]`, `0`, `false` defaults are free), and a non-zero constant default (`state { n: Integer = 5 }`) is written after zeroing;
