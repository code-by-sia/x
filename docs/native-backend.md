...
- `Number` (IEEE-754 double) arithmetic: a literal is materialized into a stack slot, and `+ - * /` are the hardware double instructions over the FP registers; `Number` parameters, returns, arguments, and fields all travel in `d`-registers per the calling convention, in a register bank separate from the integer arguments (so `f(a: Integer, x: Number, b: Integer)` puts `a`/`b` in `x`-registers and `x` in a `d`-register);
...
| `Number` (double) arithmetic, parameters, returns, and fields | done |
