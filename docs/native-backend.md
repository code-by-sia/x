...
- array parameters: a function may take an `Integer[]` or an interface array by value (the `{ data, len, cap }` fat pointer, three registers / three slots), so `mapper total(xs: Integer[])` and array extension methods both work;
- arrays with elements of any width: `Integer[]` (one slot), `String[]` (a pointer plus a length, two slots), and arrays of a compound type (laid out inline) all share the `{ data, len, cap }` fat pointer; literals, `.len`, `.data[i]` indexing, `for x in`, and by-value parameters store and load each element at `index * elementWidth`;
